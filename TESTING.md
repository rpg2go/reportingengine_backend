...
This document defines the testing architecture, coding patterns, naming conventions, and best practices for writing automated tests in this codebase. Any future LLM or developer adding tests to this project **MUST** follow these guidelines.
...
* **Unit Tests**: Suffixed with `Test`, e.g., [DateUtilsTest](src/test/java/com/db/reporting/service/DateUtilsTest.java) for [DateUtils](src/main/java/com/db/reporting/service/DateUtils.java).
* **Integration Tests**: Suffixed with `IT`, e.g., [ReportConfigServiceIT](src/test/java/com/db/reporting/it/service/ReportConfigServiceIT.java) for [ReportConfigService](src/main/java/com/db/reporting/service/ReportConfigService.java).
...
1. **Extend BaseIT**:
   Always extend [BaseIT](src/test/java/com/db/reporting/it/BaseIT.java). This handles starting the Testcontainers PostgreSQL DB or falling back to the local database, applying migrations, and loading properties.
...
1. **Empty Context Short-Circuit in Formula Evaluation**:
   In [PostProcessorService](src/main/java/com/db/reporting/service/PostProcessorService.java), `evaluateFormula(formula, context)` returns `0.0` immediately if the context map is empty. When testing simple math expressions that do not require variables, **always pass a dummy key-value pair in the context** (e.g. `Map.of("dummy", 0.0)`) to avoid this guard check.
