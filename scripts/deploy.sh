...
gcloud run deploy ${BACKEND_SERVICE_NAME} \
  ...
  --port 8080 \
  --memory 1Gi \
  --set-env-vars="^;^SPRING_DATASOURCE_URL=${SPRING_DATASOURCE_URL};SPRING_DATASOURCE_USERNAME=${SPRING_DATASOURCE_USERNAME};SPRING_DATASOURCE_PASSWORD=${SPRING_DATASOURCE_PASSWORD};SECURITY_ADMIN_USERNAME=${SECURITY_ADMIN_USERNAME};SECURITY_ADMIN_PASSWORD=${SECURITY_ADMIN_PASSWORD};SECURITY_ADMIN_ROLE=${SECURITY_ADMIN_ROLE};CORS_ALLOWED_ORIGINS=${CORS_ALLOWED_ORIGINS};SPRING_PROFILES_ACTIVE=dev" \
