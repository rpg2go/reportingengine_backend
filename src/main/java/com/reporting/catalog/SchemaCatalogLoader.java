 ...
 * tables ({@code reporting.meta_table}, {@code reporting.meta_column},
 * {@code reporting.meta_relationship}) via direct JDBC queries and assembles
...
public class SchemaCatalogLoader {
    ...
    @PostConstruct
    public void load() {
        log.info("SchemaCatalogLoader: starting catalog load from reporting.meta_* tables...");
        try {
            loadTables();
            loadColumns();
            loadRelationships();
    ...
    /**
     * Queries {@code reporting.meta_column} and attaches each column to its
