package com.db.reporting.it.service;

import com.db.reporting.it.BaseIT;
import com.db.reporting.service.ReportConfigService;
import com.db.reporting.dto.ColumnDefDto;
import com.db.reporting.dto.Enums;
import com.db.reporting.dto.ReportConfigDto;
import com.db.reporting.dto.ReportRowDto;
import com.db.reporting.dto.MeasureDefinitionDTO;
...
public class ReportConfigServiceIT extends BaseIT {
    ...
    public void saveAndLoadConfig_shouldBeConsistent() {
        ...
        List<ReportRowDto> rows = List.of(
                new ReportRowDto("R1", reportId, "Gross Sales", Enums.RowType.data,
                        new MeasureDefinitionDTO("raw", null, null, null, "SUM(amount)"), null, "normal", 0, 1,
                        Set.of("C1", "C2"), "region_id = 1"),
                new ReportRowDto("R2", reportId, "Spacer Row", Enums.RowType.blank,
                        new MeasureDefinitionDTO("raw", null, null, null, ""), null, "normal", 0, 2, Set.of(), null));

        ReportConfigDto config = new ReportConfigDto(
                reportId, "Integration Test Report", columns, rows, LocalDate.of(2026, 5, 26), 1,
                Enums.ReportStatus.draft,
                "weekly", null, null, false, null, null);
    ...
    public void saveConfig_shouldPerformCascadeDeletionsOnUpdate() {
        ...
        List<ReportRowDto> rows1 = List.of(new ReportRowDto("R1", reportId, "R1", Enums.RowType.data,
                new MeasureDefinitionDTO("raw", null, null, null, "SUM(a)"), null, "normal", 0, 1, Set.of("C1"), null));
        ReportConfigDto config1 = new ReportConfigDto(reportId, "First save", cols1, rows1, LocalDate.now(), 1, null,
                "w", null, null, false, null, null);
        config1.setSourceTable("a");
        configService.saveToDb(config1);

        // Verify rows exist in DB
        Integer rowCountBefore = jdbcTemplate
                .queryForObject("SELECT COUNT(*) FROM report_builder_owner.row_definition WHERE report_id = ?", Integer.class, reportId);
        assertThat(rowCountBefore).isEqualTo(1);

        // Act: Save updated configuration without any rows
        ReportConfigDto config2 = new ReportConfigDto(reportId, "Second save", cols1, List.of(), LocalDate.now(), 1,
                null, "w", null, null, false, null, null);
        config2.setSourceTable("a");
        ...
        Integer mapCountAfter = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM report_builder_owner.row_column_intersection WHERE report_id = ?", Integer.class, reportId);
